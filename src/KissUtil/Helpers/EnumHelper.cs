...
namespace KissUtil.Helpers
{
    ...
    public static class EnumHelper {
        ...
        /// </summary>
        /// <param name="type">枚举类型</param>
        public static List<EnumItem> GetItems( Type type ) {
        ...
        /// 添加描述项
        /// </summary>
        private static void AddItem( Type type, ICollection<EnumItem> result, FieldInfo field ) {
            if( !field.FieldType.IsEnum )
                return;
            var value = GetValue( type, field.Name );
            var description = Reflection.GetDescription( field );
            result.Add( new EnumItem( description, value, value ) );
        }
